/*!
    ...
    \author Шпагин Д.
    \date 5 дек. 2023 г.
    Установите директиву USING_LOGGING в этом файле, чтобы открыть код логгера.
    ...
    int main()
    {
        LOGGER_INIT_FILE(true, LEVEL_INFO, "C:\\my_path");
        while (true)
        {
            // We're doing something
            LOG_INFO("All right, %s", "I'm okay.");
            // We're doing something
        }
        return 0;
    }
    \endcode
    \warning Потокобезопасной является только функция logging, все остальные не потокобезопасны.
    \warning Подразумевается, что основное управление логгером будет только происходить из родительского потока.
*/
